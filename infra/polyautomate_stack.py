...
POLYMARKET_PRIVATE_KEY="$(printf '%s' "$SECRET_JSON" | python3 -c 'import json,sys; print(json.load(sys.stdin).get("POLYMARKET_PRIVATE_KEY", ""))')"
...
if [[ "$DESIRED_SIG" != "$CURRENT_SIG" ]]; then
  ...
  docker run -d --name polyautomate-executor --restart unless-stopped \
    ...
    -e LONGSHOT_MAX_SPREAD="$LONGSHOT_MAX_SPREAD" \
    -e LONGSHOT_MAX_REL_SPREAD="$LONGSHOT_MAX_REL_SPREAD" \
    -e LONGSHOT_HOLD_GRACE_HOURS="$LONGSHOT_HOLD_GRACE_HOURS" \
    ...
    -e LONGSHOT_GUARDRAIL_ENABLED="$LONGSHOT_GUARDRAIL_ENABLED" \
    -e LONGSHOT_GUARDRAIL_WINDOW_TRADES="$LONGSHOT_GUARDRAIL_WINDOW_TRADES" \
    -e LONGSHOT_GUARDRAIL_MIN_TRADES="$LONGSHOT_GUARDRAIL_MIN_TRADES" \
    -e LONGSHOT_GUARDRAIL_MIN_PNL_USD="$LONGSHOT_GUARDRAIL_MIN_PNL_USD" \
    -e LONGSHOT_GUARDRAIL_MIN_WIN_RATE="$LONGSHOT_GUARDRAIL_MIN_WIN_RATE" \
    -e LONGSHOT_GUARDRAIL_COOLDOWN_MIN="$LONGSHOT_GUARDRAIL_COOLDOWN_MIN" \
...
SCRIPT""",
            ...
            "echo 'LONGSHOT_MAX_REL_SPREAD=0.15' >> /etc/polyautomate-executor.env",
            "echo 'LONGSHOT_HOLD_GRACE_HOURS=24' >> /etc/polyautomate-executor.env",
            ...
            "echo 'LONGSHOT_GUARDRAIL_ENABLED=1' >> /etc/polyautomate-executor.env",
            "echo 'LONGSHOT_GUARDRAIL_WINDOW_TRADES=12' >> /etc/polyautomate-executor.env",
            "echo 'LONGSHOT_GUARDRAIL_MIN_TRADES=4' >> /etc/polyautomate-executor.env",
            "echo 'LONGSHOT_GUARDRAIL_MIN_PNL_USD=-5' >> /etc/polyautomate-executor.env",
            "echo 'LONGSHOT_GUARDRAIL_MIN_WIN_RATE=0.35' >> /etc/polyautomate-executor.env",
            "echo 'LONGSHOT_GUARDRAIL_COOLDOWN_MIN=180' >> /etc/polyautomate-executor.env",
            ...
            "echo portfolio_publisher_userdata_version=4",
...
if docker inspect polyautomate-executor >/dev/null 2>&1; then
  docker cp polyautomate-executor:/var/lib/polyautomate/longshot-state.json "$STATE_DIR/longshot-state.json" >/dev/null 2>&1 || true
  if docker exec polyautomate-executor python -m polyautomate.portfolio \
    --user "$POLYMARKET_ADDRESS" \
    --include-clob-balance \
    --out /tmp/polyautomate-portfolio.html; then
    docker cp polyautomate-executor:/tmp/polyautomate-portfolio.html "$STATE_DIR/portfolio/index.html"
  else
    echo "portfolio_publish_container_failed"
    exit 0
  fi
elif PYTHONPATH="$REPO_DIR" python3 -c 'import polyautomate.portfolio' >/dev/null 2>&1; then
  PYTHONPATH="$REPO_DIR" python3 -m polyautomate.portfolio \
    --user "$POLYMARKET_ADDRESS" \
    --include-clob-balance \
    --out "$STATE_DIR/portfolio/index.html"
else
  echo "portfolio_publish_skipped reason=no_container_and_host_module_unavailable repo_dir=$REPO_DIR"
  exit 0
fi
aws s3 cp "$STATE_DIR/portfolio/index.html" "s3://$PORTFOLIO_BUCKET/index.html" \
  --region "$REGION" \
...
SCRIPT""",
                    ...
                    "echo '* * * * * root bash -lc \"set -a; source /etc/polyautomate-portfolio.env; set +a; /usr/local/bin/publish-portfolio-dashboard.sh >> /var/log/polyautomate-portfolio.log 2>&1; aws s3 cp /var/log/polyautomate-portfolio.log s3://$PORTFOLIO_BUCKET/publisher.log --region $REGION --content-type text/plain --cache-control no-store --only-show-errors || true\"' > /etc/cron.d/polyautomate-portfolio",
                    "chmod 644 /etc/cron.d/polyautomate-portfolio",
                    "echo portfolio_publisher_version=7",
